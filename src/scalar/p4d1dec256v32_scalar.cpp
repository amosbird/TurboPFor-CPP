// Scalar implementation of P4 decoding for 256v32 format
//
// This is the decode counterpart to p4enc256v32_scalar.cpp.
// Decodes P4-encoded data with 256v32 8-lane interleaved bitpacking format
...
//
// Payload depends on encoding strategy (see p4enc256v32_scalar.cpp for details)
// Bitpacking uses 256v32 format: 8-lane interleaved horizontal packing
...
void applyDelta1_256(uint32_t * out, unsigned n, uint32_t start)
{
    if (n == 0u)
        return;
...
// Decode P4 payload with bitmap exceptions for 256v32 format
//
// Format: [bitmap][patch bits][base bits]
//
// Parameters:
//   in: Input buffer positioned at bitmap
//   n: Number of elements (typically 256)
//   out: Output array
...
const unsigned char * p4D1Dec256PayloadBitmap(
...
// Decode P4 payload for 256v32 format
//
...
//
// Returns: Pointer past end of consumed input
const unsigned char * p4D1Dec256Payload(
    const unsigned char * in, unsigned n, uint32_t * out, uint32_t start, unsigned b, unsigned bx)
{
    using namespace turbopfor::scalar::detail;
    ...
    return p4D1Dec256PayloadBitmap(in, n, out, start, b, bx);
}

} // namespace

// Main P4 decoding entry point for 256v32 format (scalar implementation)
//
// Decodes P4-encoded data and applies delta1 decoding.
// This is the decode counterpart to p4Enc256v32.
//
// Parameters:
//   in: Input buffer containing P4-encoded data
//   n: Number of values to decode (typically 256)
...
unsigned char * p4D1Dec256v32(unsigned char * in, unsigned n, uint32_t * out, uint32_t start)
{
    ...
    // Unpack base values (256v32 format)
