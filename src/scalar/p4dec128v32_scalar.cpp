...
const unsigned char * p4Dec128PayloadBitmap(
...
const unsigned char * p4Dec128Payload(
    const unsigned char * in, unsigned n, uint32_t * out, unsigned b, unsigned bx)
{
    using namespace turbopfor::scalar::detail;
    ...
    if ((b & 0x80u) == 0u)
    {
        return bitunpack128v32Scalar(in, out, b);
    }
    ...
    return p4Dec128PayloadBitmap(in, n, out, b, bx);
}
...
const unsigned char * p4Dec128v32(const unsigned char * in, unsigned n, uint32_t * out)
{
    ...
    if ((b & 0x40u) == 0u)
    {
        ...
        return p4Dec128Payload(ip, n, out, b, bx);
    }
